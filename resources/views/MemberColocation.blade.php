        ...
        <div class="max-w-6xl mx-auto px-6">
            ...
            <div class="flex justify-between items-center mb-6">
                ...
                <a href="{{ route('admin.colocations.index') }}"
                    class="bg-gray-800 hover:bg-gray-900 text-white px-5 py-2 rounded-lg shadow transition">
                    ← Dashboard
                </a>
                ...
                <form method="POST" action="{{ route('colocation.leave', $colocation->id) }}">
                    @csrf
                    @method('DELETE')
                    <button type="submit" onclick="return confirm('Êtes-vous sûr de vouloir quitter cette colocation ?')"
            ...
            <div class="bg-white rounded-2xl shadow-md p-6">
                ...
                <div class="space-y-2">
                    @forelse($dettes as $dette)
                        <p class="text-gray-700">{{ $dette->debiteur->name }} doit {{ $dette->creancier->name }}
                            {{ $dette->montant }}dh</p>
                    @empty
